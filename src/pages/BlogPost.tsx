...
export function BlogPost() {
  ...
  const handleConsult = useCallback(() => {
    openConsultation({
      source: "blogpost-cta",
      context: { postId: post?.id ?? id ?? null, postTitle: post?.title ?? null },
    });
  }, [openConsultation, post?.id, post?.title, id]);
  if (!post) {
    return (
        ...
        <div className="py-8 md:py-10 lg:py-12">
          <div className="py-24 md:py-32 text-center">
            ...
            <p className="text-muted-foreground mb-10 max-w-md mx-auto">
              The requested legal article has been archived or its location has changed.
            </p>
            <Link to="/blog">
          ...
          <FullBleed>
            <header className="relative h-[70vh] flex items-end pb-20 overflow-hidden">
              ...
              <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 w-full relative z-10">
                <Link
                  to="/blog"
                  className="inline-flex items-center text-white/70 hover:text-[#B45309] transition-colors mb-10 group text-sm font-bold uppercase tracking-widest"
                >
                ...
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div className="flex items-center gap-4 text-white/60 text-xs font-bold uppercase tracking-widest">
                    <span className="flex items-center gap-2">
                      <Calendar className="w-4 h-4 text-[#B45309]" /> {post.date}
                    </span>
                    <span className="flex items-center gap-2">
                      <Clock className="w-4 h-4 text-[#B45309]" /> 6 min read
                    </span>
                  </div>
                </div>
              ...
              <aside className="lg:col-span-3 lg:sticky lg:top-32 h-fit space-y-16">
                ...
                <div className="pt-10 border-t border-border/50">
                  ...
                  <div className="flex gap-3">
                    <Button variant="ghost" size="icon" className="hover:text-[#B45309] hover:bg-[#B45309]/5 transition-all" aria-label="Share on X">
                      <Twitter className="w-5 h-5" />
                    </Button>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="hover:text-[#B45309] hover:bg-[#B45309]/5 transition-all"
                    ...
                    <Button variant="ghost" size="icon" className="hover:text-[#B45309] hover:bg-[#B45309]/5 transition-all" aria-label="Copy share link">
                  ...
                  <div className="bg-[#0F172A] dark:bg-slate-900 rounded-[3rem] p-12 flex flex-col md:flex-row items-center justify-between gap-10 shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="max-w-lg relative z-10">
                      ...
                      <p className="text-slate-400 font-light leading-relaxed">
                        Our senior practice partners provide tailored advice for multinational enterprises navigating the complexities mentioned above.
                      </p>
                    </div>
                    <Button
                      size="lg"
                      onClick={handleConsult}
                      className="rounded-full px-12 h-16 bg-[#B45309] hover:bg-[#92400E] text-white border-none shadow-glow font-bold relative z-10"
                    >
